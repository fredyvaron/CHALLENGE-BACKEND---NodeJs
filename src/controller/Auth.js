...
const erroresLogin = Joi.object({
    email: Joi.string().min(6).max(255).required().email(),
    clave: Joi.string().min(6).max(1024).required()
})
async function login(req, res, next){
    const { error } = erroresLogin.validate(req.body);
    if(error){
    ...
    try {
        ...
        if(usuario){
            const validaclave = await bcrypt.compare(req.body.clave, usuario.clave);
            if(validaclave){
...
function generateAccessToken(nombre) {
    return jwt.sign(nombre, process.env.TOKEN_SECRET, { expiresIn: '1800s' });
  }
